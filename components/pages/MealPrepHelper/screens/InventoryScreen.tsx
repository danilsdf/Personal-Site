	...
	return (
		<div>
			<section className="mb-10 text-center">
				...
				<p className="text-sm text-neutral-700 dark:text-neutral-200 mb-4">Add your available ingredients for the week.</p>
			</section>
			<section className="mb-6 flex flex-col items-center gap-2">
				<div className="flex gap-2">
					<button
						type="button"
						className="rounded bg-yellow-600 dark:bg-yellow-400 px-3 py-1 text-xs font-semibold text-white dark:text-black hover:bg-yellow-700 dark:hover:bg-yellow-300"
						onClick={handleAdd}
					>
						Add Ingredient
					</button>
			...
			{showModal && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
					<div className="bg-white dark:bg-neutral-900 rounded-lg p-6 w-full max-w-md shadow-lg">
						<h3 className="text-lg font-bold mb-2">Add Ingredient</h3>
						{!selectedIngredient ? (
							<>
								<input
									type="text"
								...
								<ul className="max-h-40 overflow-y-auto mb-2">
									...
									{filteredIngredients.map(i => (
										<li
											...
											onClick={() => setSelectedIngredient(i.name)}
										>
											{i.name}
										</li>
									))}
							...
							<>
								<div className="mb-2">
									...
									<button className="ml-2 text-xs text-yellow-600 underline" onClick={() => setSelectedIngredient(null)}>Change</button>
								</div>
								<div className="mb-2">
									<label className="text-sm mr-2">Unit:</label>
									<select
										value={unit}
										onChange={e => setUnit(e.target.value)}
										className="rounded border px-2 py-1 text-sm bg-neutral-50 dark:bg-neutral-800"
									>
								...
								<div className="mb-2">
									...
									<input
										...
										className="rounded border px-2 py-1 text-sm bg-neutral-50 dark:bg-neutral-800 w-24"
									/>
								</div>
